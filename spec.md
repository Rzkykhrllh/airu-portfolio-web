...
- **Shared form subcomponents.** `PhotoAddModal` and `PhotoEditModal` still duplicate ~150 lines of near-identical EXIF/collections/visibility form markup. Extracting shared `<ExifFields>`/`<CollectionsChecklist>`/`<VisibilitySelect>` components was scoped during the audit but not done this batch — code-quality cleanup rather than something that changes how the admin works day-to-day, and riskier to do quickly across two already-complex forms. Worth picking up on its own pass.
- **ESLint.** Discovered this session that the repo has no ESLint config at all (`next lint` prompts to create one from scratch) — not something to set up unattended as a side effect of an unrelated task. Flagging for whenever the owner wants it.
