...
- **Site-wide traffic analytics ✅ Done (2026-08-17).** Owner skipped the Plausible/Umami-vs-GA4 comparison and went straight to GA4 for cross-project familiarity. See §11 for the implementation.
...
- **Admin bulk actions** (select multiple → bulk delete/move-to-collection/change visibility). ✅ **Done** (2026-08-16) as part of the full admin-panel audit — see §10.
...
## 10. Admin Panel Improvements (2026-08-16)

Owner asked for a full exploratory audit of the admin panel ("halaman admin") before proposing fixes, then approved fixing everything found, prioritized toward whatever makes day-to-day photo management easiest. Audit combined static code reading with live browser testing against `byairu.com/admin` (Claude-in-Chrome) — one live-only finding (library had grown to 538 photos, all fetched in a single `limit: 1000` request) and one candidate finding (photo-card clicks appearing to do nothing) were caught this way; the click issue was cross-verified via direct JS-context `element.click()` and confirmed to be a synthetic-click quirk of the browser-automation tool itself, not a real bug — excluded from the findings, not fixed.

### Built (`airu-porto-fe` commit `7621704`)
- **Real pagination for the admin Photos list.** `getPhotosPage()` (`lib/api.ts`) extended to support `scope: 'admin'` via `adminFetchWithMeta`. `/admin/photos` now pages at 60 photos/request with Previous/Next controls, instead of fetching up to 1000 photos in one request on every load/filter change.
- **Bulk actions.** New `components/admin/BulkActionBar.tsx` + select-mode support added to `PhotoGrid`/`PhotoList`/their item components (checkbox overlay, click-to-toggle when active). Supports add-to-collection, set-visibility, and delete across however many photos are selected. Backend has no bulk endpoints, so each action fans out one request per photo and reports partial success/failure rather than failing the whole batch on the first error. Add-to-collection correctly merges into each photo's existing `collections` list rather than overwriting it — `updatePhoto`'s `collectionIds` payload replaces a photo's full membership, so bulk-add reads each photo's current collections from already-loaded state and merges the target in.
- **Shared `ConfirmDialog`** (`components/providers/ConfirmDialogProvider.tsx`, promise-based, matches the existing `ToastProvider` pattern) replacing native `confirm()` at all 6 call sites: `PhotoGrid`, `PhotoListItem`, `PhotoEditModal`, `CollectionPhotoTile`, `admin/collections/page.tsx`, `admin/inquiries/page.tsx`. `components/ui/Button.tsx` converted to `React.forwardRef` so the dialog can autofocus its confirm button.
- **Real backend error messages surfaced in the UI.** Toasts across the admin panel previously showed a hardcoded generic string regardless of what the backend actually said (validation errors, rate limits, etc. were all invisible). Now `error instanceof ApiError ? error.message : '<fallback>'` throughout.
- **Graceful session-expiry handling.** Admin JWTs expire after 1 week with no refresh flow; hitting a stale session previously surfaced as a confusing "Failed to load" error from whichever component happened to call the API next. `lib/fetch.ts` now clears the stored token and redirects to `/admin/login` on any 401/403 from `adminFetch`/`adminFetchWithMeta`/`uploadFetch`.
- **Upload modal fixes.** `PhotoAddModal`'s drag-and-drop text was decorative — no `onDrop`/`onDragOver` handlers existed, so dragging a file onto the dropzone did nothing. Now fully wired, with drag-over visual feedback. Size-limit text said "up to 10MB"; the real backend limit (multer, `upload.middleware.ts`) is 50MB — text corrected and a client-side pre-flight size check added so oversized files are rejected immediately with a clear message instead of failing after a full upload attempt.
- **Paginated "Add Photos" picker** in the collection editor (`admin/collections/[slug]/page.tsx`) — was rendering every client-side search match at once (up to hundreds of thumbnails for an empty search against 538 photos); now caps at 60 with a "Load more" button, reset on search change.
- **De-duplicated optimistic toggle logic.** `PhotoGrid`, `PhotoListItem`, and `CollectionPhotoTile` each had a near-identical copy-pasted implementation of "set state → `updatePhoto` → roll back + toast on failure" for the featured/visibility quick-toggle controls. Extracted into `hooks/usePhotoQuickEdit.ts`.
- **Removed `hooks/useRequireAuth.ts`** — confirmed via grep to be completely unused dead code, duplicating the auth check `AdminLayout.tsx` already does inline.

`tsc --noEmit` clean (full project). `next build`/lint not run this session — device-bridge shell calls are stateless per-call (background processes don't persist) and capped at 45s, which isn't enough for a full production build; typecheck plus manual review of each diff was the verification available here. Not yet verified live — needs push + Dokploy deploy (owner pushes manually, see §9's push-automation note — still not automatable from this session).

### Follow-up batch, same day: undo-delete safety net, shared form components, ESLint

Owner picked two of four proposed follow-ups (bulk-delete safety net; shared-form cleanup + ESLint) after confirming a separate ask — easy per-photo visibility/featured toggles — already existed in the code shipped above (`VisibilityMenu`/`FeaturedToggle` on every grid/list card) and just hadn't been deployed yet.

**Undo-delete safety net** (`airu-porto-fe` commit `36f3000`). Bulk delete (above) raised the stakes of a misclick — one confirm dialog was the only thing between a fat-fingered selection and permanently losing a lot of photos. Added a 6-second grace period before any photo delete actually reaches the API: the photo (or selection) is hidden immediately, an "Undo" toast appears, and the real `DELETE` only fires if the toast times out unclaimed. New `components/providers/UndoDeleteProvider.tsx` centralizes the timer at the provider level (mounted in `app/admin/layout.tsx`, inside `ToastProvider` / outside `ConfirmDialogProvider`) rather than as per-component state — this matters specifically for `PhotoEditModal`, which now closes immediately on delete instead of showing its own spinner, and a component-local timer would've died with the unmount. `ToastProvider` gained an `undoable(message, onUndo, duration)` variant with an inline action button (`components/ui/Toast.tsx` now supports an optional `action`). Applies to `PhotoGrid`'s and `PhotoListItem`'s own delete buttons, `PhotoEditModal`'s delete, and bulk delete (`admin/photos/page.tsx`, via a `pendingDeleteIds` set that filters the rendered list). `CollectionPhotoTile`'s "remove from collection" was deliberately left as an instant action — it doesn't delete the photo, just its membership in one collection, trivially reversible via Add Photos, so didn't need the same treatment. If the tab closes before the grace period elapses, the delete simply never fires (server-side state is untouched) — an intentionally safe failure mode.

**Shared form components** (`airu-porto-fe` commit `df23465`, closes the deferred item below). Extracted `VisibilitySelect`, `CollectionsChecklist`, `FeaturedCheckbox`, and `ExifFields` out of `PhotoAddModal`/`PhotoEditModal`'s duplicated form markup — cut `PhotoAddModal` by ~130 lines and `PhotoEditModal` by ~150. `PhotoEditModal`'s five separate camera/lens/aperture/shutter/iso `useState` calls collapsed into one `exif` object matching the shape `PhotoAddModal`'s `formData.exif` already used, so both modals now share the exact same `ExifFields` props contract.

**ESLint** (`airu-porto-fe` commit `12a451f`, closes the deferred item below). Added `.eslintrc.json` (`next/core-web-vitals` + `next/typescript`, matching Next's own "Strict" preset — the repo had no config at all before this). Fixed what it flagged in the admin files touched this session: an unescaped apostrophe in both modals, a `useRouter`/`router` that turned out to be fully dead in the collection editor, an unused `Link` import, and a couple of `react-hooks/exhaustive-deps` warnings suppressed with an explanatory comment (the omitted `toast`/`loadData`/`loadInquiries` values are new references every render, so including them would cause extra fetches rather than fix anything real). Pre-existing lint issues outside the admin panel (`any`-typed API helpers in `lib/api.ts`/`lib/transformers.ts`, `<img>` vs `next/image` in a few public-facing components, unescaped entities on `/`, `/about`, `not-found`, `error` pages, a `next-env.d.ts` triple-slash reference) are untouched — out of scope for this pass, left for whenever those areas get worked on next. One remaining warning inside `app/admin/` itself: an unused `err` in `admin/login/page.tsx`'s catch block, left alone since that file wasn't otherwise touched this session.

`tsc --noEmit` clean and `eslint app/admin components/admin` clean (bar the one pre-existing `admin/login` warning noted above) after each of these three commits. Same verification ceiling as the batch above — no full `next build` run this session, device-bridge shell calls are stateless per-call and capped at 45s.

### Deferred, still open
- **Multi-file upload.** `PhotoAddModal` only accepts one file per open. Proposed alongside the above but not picked this round — relevant given the owner mentioned wanting to bulk-upload photos soon.
- **Duplicate photo check.** Live testing during the original audit turned up what looked like duplicate photos in the 538-photo library. Never confirmed or investigated further — proposed but not picked this round.

---

## 11. Google Analytics 4 with Consent Banner (2026-08-17)

Closes the "Site-wide traffic analytics" deferred item from §6. Owner skipped the Plausible/Umami-vs-GA4 comparison and asked to go straight to GA4, plus explicitly asked for a consent banner rather than firing GA4 unconditionally.

**Implementation** (`airu-porto-fe` commit `c983af5`). Native `next/script` (two `afterInteractive` tags: the `gtag.js` loader + an inline init snippet) instead of `@next/third-parties/google` — the latter would add a new npm dependency, and this session's device-bridge shell has no network access to run `npm install` on the owner's machine, which would've left them needing to install it manually after pulling.

Everything is gated behind a client-side consent state, stored in `localStorage` under `analyticsConsent` (`lib/config.ts`'s `STORAGE_KEYS`):
- New `components/analytics/AnalyticsConsent.tsx`, mounted once in `app/layout.tsx` (inside `<body>`, alongside `ThemeProvider`/`ConditionalLayout`). Renders `null` entirely — no banner, no script tag, nothing — if `NEXT_PUBLIC_GA_MEASUREMENT_ID` (`lib/config.ts`'s `GA_MEASUREMENT_ID`, sourced from the env var) is unset. This makes the feature safe to ship even before the GA4 property exists.
- Once the env var is set: first-time visitors see a bottom-of-screen banner ("Accept" / "Decline"). The GA4 scripts only render — and only then does a single request to `googletagmanager.com` fire — after "Accept" is clicked. The choice is remembered in `localStorage` so the banner doesn't reappear on later visits, and "Decline" also suppresses it (recorded as `denied`, never re-prompted).
- This is a stricter "nothing loads before opt-in" approach than Google's own default-denied Consent Mode (which still pings before consent) — deliberate, since a low-traffic personal site has no consent-management platform and this is the simplest way to stay unambiguously GDPR-safe.

`tsc --noEmit` and `eslint` (scoped to the touched files) both clean. Same verification ceiling as prior batches — no full `next build` run this session.

**Still owner's own follow-through, not code** — this needs the owner's own Google login, which nothing in this session can do on their behalf:
1. Go to [analytics.google.com](https://analytics.google.com), sign in, click Admin (gear icon, bottom left) → **Create Property**.
2. Name it (e.g. "Airu Photography"), set timezone/currency, click through the business-details prompts (any answer works, they're just for Google's benchmarking).
3. Under "Choose a platform," pick **Web**. Enter the site URL (`https://byairu.com`) and a stream name.
4. Google creates a **Data Stream** and shows a **Measurement ID** shaped like `G-XXXXXXXXXX` — that's the value this feature needs.
5. On the device, add `NEXT_PUBLIC_GA_MEASUREMENT_ID=G-XXXXXXXXXX` to `.env.local` (gitignored, not `.env.example` — real IDs don't belong in version control even though a Measurement ID isn't especially sensitive). This session couldn't write `.env.local` directly — remote-write tooling blocks that filename outright, even for a non-secret value like this — so it's a manual add.

**Done (2026-08-17):** owner created the property and got Measurement ID `G-C7Z134R04B`.

**Deploy gotcha found, not yet fixed — owner chose to handle manually in Dokploy rather than have this session touch the Docker build pipeline:** `NEXT_PUBLIC_*` vars are inlined into the client bundle at `npm run build` time. This repo's `Dockerfile` doesn't declare any build `ARG`s, and `docker-compose.yml`'s `environment:` block only takes effect at container *runtime* — after the build (and thus the inlining) has already happened. `.env.local` is also excluded from the Docker build context via `.dockerignore`. Net effect: simply setting `NEXT_PUBLIC_GA_MEASUREMENT_ID` in `.env.local` or in Dokploy's runtime environment section will **not** get it into the production bundle — the banner/script will silently stay off in prod even though local dev works. Owner needs to get the value into the Docker **build** step specifically (e.g. Dokploy's build-time/build-arg env config, if it has one, or by adding a build `ARG`/`ENV` to the `Dockerfile` and passing it via `docker-compose.yml`'s `build.args`). Flagging this explicitly so it doesn't read as "should just work after a normal deploy" — it's the same category of gap `NEXT_PUBLIC_API_BASE_URL` has, just never surfaced before because that one's fallback default (`/api`) happens to already match what prod needs.
6. Once the build actually receives the env var, the banner will start showing to new visitors, and Realtime reports in the GA4 dashboard should show activity within a minute or two of someone accepting.

Push + Dokploy deploy is, as with every batch this session, a manual step for the owner — `git push` remains unautomatable from this session (proxy blocks GitHub from the device-bridge shell over both HTTPS and SSH; token-based HTTPS push is off the table per policy, since Claude must never handle credentials).

---

## 12. Clickable tags + `/tags/[tag]` browse page (2026-08-18)

Owner asked for tags to be clickable and query other photos sharing the same tag. Investigation before building: the backend already had everything needed on that side — `GET /photos?tag=xxx` (`photo.controller.ts`) does `where.tags = { some: { tag } }`, ANDed with the existing `where.visibility` scope filter, so a public request with a tag always stays PUBLIC-only. `lib/api.ts`'s `getPhotos`/`getPhotosPage` already accepted a `filters.tags` array and threaded it through as `?tag=`. The actual gap was 100% frontend: tags rendered as plain `<span>` badges on the photo detail page (already hover-styled, just never wired to anything), and there was no page anywhere that could show "all photos with tag X."

**Design choice — dedicated route vs. homepage query param:** owner's first instinct was reusing the homepage gallery with the tag as a filter. Weighed both: a `?tag=` query param on `/` would've meant `GalleryView` (currently 100% client-state, never reads any URL param on load — a deliberate prior design noted in §8) taking on URL-driven state for the first time, and query-string pages don't get their own metadata/OG image or show up cleanly for sharing — a real cost given how much of this session (and this owner generally) has gone into photo discoverability/SEO. A dedicated `/tags/[tag]` route costs barely more code (mirrors the existing `/collections/[slug]/page.tsx` pattern almost exactly) while getting a clean shareable URL, its own `generateMetadata` (title/OG image drawn from the tag's first matching photo), and a natural 404 when a tag matches nothing. Went with the dedicated route.

**Built** (`airu-porto-fe` commit `5cad53a`):
- `lib/data.ts`: `GalleryFilters` gained `tag?: string`, threaded into `getGalleryPage`'s call to `getPhotosPage` as `tags: [tag]`.
- `components/gallery/GalleryView.tsx`: new optional `lockedTag` prop. When set, every fetch (initial SSR page, infinite-scroll `loadMore`, and the filter-changed effect) includes the tag on top of whatever search/collection/featured filters the visitor also has active — so tag + search/collection/featured compose rather than being mutually exclusive. Header shows "Tagged: {tag}" with an "All photographs" link back to `/` instead of the usual "Gallery" heading. Empty state extended to cover "0 results for this tag" (previously only triggered by the user-facing search/collection/featured filters, which don't count `lockedTag` as one of them since it's not user-clearable from this view).
- New `app/tags/[tag]/page.tsx`: SSR's the first page via `getGalleryPage(1, { tag })`, `generateMetadata` builds a tag-specific title/description/canonical/OG image (cover = first matching photo), calls `notFound()` when zero photos match — same call `/collections/[slug]` makes for a missing slug, since there's no way to distinguish "unknown tag" from "tag exists but nothing PUBLIC currently has it" and both mean nothing to show.
- `app/photo/[id]/page.tsx`: tag badges converted from `<span>` to `<Link href="/tags/{encodeURIComponent(tag)}">`, same existing hover styling.

`tsc --noEmit` clean; `eslint` clean on all touched files (one pre-existing unrelated warning in this same file, `'getAllPhotos' is defined but never used`, left alone — not touched this session, already present before). Not yet verified live — needs push + Dokploy deploy.

**Scope notes / not done:** tags are only clickable from the photo detail page (the only place they're currently shown publicly) — not added to `PhotoCard`/gallery grid cards, since they don't render tags at all today. Tag pages aren't added to `sitemap.xml` — worth reconsidering once there's a sense of how many distinct tags exist and whether they're substantial enough to be worth indexing individually (possible follow-up, not blocking).
