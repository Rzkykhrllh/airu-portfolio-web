...
5. On the device, add `NEXT_PUBLIC_GA_MEASUREMENT_ID=G-XXXXXXXXXX` to `.env.local` (gitignored, not `.env.example` — real IDs don't belong in version control even though a Measurement ID isn't especially sensitive).
6. Rebuild/redeploy (Dokploy). Once live, the banner will start showing to new visitors, and Realtime reports in the GA4 dashboard should show activity within a minute or two of accepting.
